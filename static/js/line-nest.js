...
! function() {
            ...
            for (var A = 0; A < a.length; A++) {
                ...
                if (t !== i && null !== i.x && null !== i.y) {
                    var z, l = t.x - i.x,
                        B = t.y - i.y,
                        n = l * l + B * B;
                    n < i.max && (i === w && n >= i.max / 2 && (t.x -= 0.03 * l, t.y -= 0.03 * B), z = (i.max - n) / i.max, p.beginPath(), p.lineWidth = z*1.3 , p.strokeStyle = "rgba(0,0,0," + (z + 0.2) + ")", p.moveTo(t.x, t.y), p.lineTo(i.x, i.y), p.stroke())
                }
            }
    ...
    h.style.zIndex = -1;
    h.style.opacity = 0.5;
    h.style.backgroundColor = "#3C3C47";

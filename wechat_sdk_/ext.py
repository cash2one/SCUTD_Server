...
class WechatExt(object):
    ...
    def update_news(self, news,news_id):
        """
        在素材库中升级图文消息
        ...
        url = 'https://mp.weixin.qq.com/cgi-bin/operate_appmsg?lang=zh_CN&t=ajax-response&sub=update&token={token}'.format(
            token=self.__token,
        )
        payload = {
            'token': self.__token,
        ...
        headers = {
            'referer': 'https://mp.weixin.qq.com/cgi-bin/appmsg?t=media/appmsg_edit&action=edit&type=10&appmsgid={appmsgid}&token={token}&lang=zh_CN'.format(
                token=self.__token,appmsgid=news_id
            ),
            'cookie': self.__cookies,
        }
        i = 0
    ...
    def upload_file(self, filepath):
        ...
        try:
            files = {'file': requests.get("https://mmbiz.qlogo.cn/mmbiz/Mr1p0S8dRnjUN0oHKnN4tUMkdmOnYdL21DW5AlEnHDbiceibul4YuJlbEqfwoQxKficlZ6IgicUYLevy1Pz8PYWBmQ/0?wx_fmt=png")}
